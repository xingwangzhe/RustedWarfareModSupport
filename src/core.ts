...
export class CoreCompletionProvider implements vscode.CompletionItemProvider {
    provideCompletionItems(
        ...
        // 检查当前是否在[core]节内
        if (!this.isInsideCoreSection(document, position)) {
            return [];
        }

        // 获取core节的属性列表
        const coreProperties = this.getCoreProperties();
    ...
    private isInsideCoreSection(document: vscode.TextDocument, position: vscode.Position): boolean {
    ...
    private getCoreProperties(): any[] {
        try {
            const corePath = path.join(__dirname, '..', 'data', 'sections', 'core.json');
            const coreData = JSON.parse(fs.readFileSync(corePath, 'utf8'));
            return coreData.data || [];
        } catch (error) {
            console.error('Error reading core.json:', error);
            return [];
        }
    }
}