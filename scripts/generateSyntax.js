...
class SyntaxGenerator {
  ...
  generateLogicExpressionPatterns() {
    return {
      patterns: [
        {
          match:
            "\\b(self|memory\\.\\w+|resource\\.\\w+|null|index|thisActionTarget|eventSource|activeWaypointTarget|customTarget[12])\\b",
          name: "variable.language.logic.ini",
        },
        {
          match:
            "\\.(x|y|z|dir|speed|maxMoveSpeed|size|teamId|id|builtAmount|hp|maxHp|maxShield|maxEnergy|height|ammo|energy|shield|kills|isFlying|isMoving|underwater|flying|completed)\\b",
          name: "variable.other.property.logic.ini",
        },
        {
          match:
            "\\b(distance|distanceSquared|min|max|sin|cos|rnd|direction|squareRoot|int|length)(?=\\()",
          name: "support.function.math.logic.ini",
        },
        {
          match:
            "\\b(nearestUnit|globalSearchForFirstUnit|attachment|transporting|numberOfUnitsInTeam|hasUnitInTeam|readUnitMemory|createMarker)(?=\\()",
    ...
    return {
      ...
      repository: {
        ...
        keyvalue: {
          ...
          patterns: [{ include: "#valueTypes" }, { include: "#comment" }],
        },
  ...
  calculateStats() {
    const stats = {
      ...
      totalProperties: Object.values(this.propertyData).reduce(
